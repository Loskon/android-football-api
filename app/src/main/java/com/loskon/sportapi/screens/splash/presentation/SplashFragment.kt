        ...
        viewModel.getMatchListState.observe(this) {
            when (it) {
                is SplashState.Success -> {
                    findNavController().navigate(SplashFragmentDirections.openMatchList(it.matches.toTypedArray()))
                }
                is SplashState.Failure -> {
                    findNavController().navigate(SplashFragmentDirections.openMatchList(null))
                }
                null -> {}
            }
        }
